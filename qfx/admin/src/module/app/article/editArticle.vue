        ...
        <div class="fwtj clearfix">
          <dl class="tj-layout">
            ...
            <dd class="tj-main">
              ...
              <div class="tj-ico">
                <el-button @click="addDomainSelect" icon="plus"><i class="add">+</i>添加</el-button>
              </div>
            </dd>
          </dl>
        </div>
...
<script>
  ...
  export default{
    ...
    methods: {
      fetchData () {
        this.$http.get(API.url + 'articles/' + this.$route.params.id).then((res) => {
          var resBody = res.body;
          var article = resBody.article;
          var servicesArr = [];
          this.article.title = article.title
          this.article.published_at = moment(article.published_at).toDate()
          this.article.source = article.source
          this.article.category = article.category
          this.article.service_category_id = article.service_category_id
          this.article.content = article.content

          article.services.forEach((v, i) => {
            servicesArr.push({value: v.category_id, name: v.id, tags3: []})
          })
          this.article.recommend = [...servicesArr]; //拷贝数组
          servicesArr.forEach((v, i) => {
            this.getRecomend(v.value, i)
          })
        })
      },
      /*返回*/
      ...
      getRecomend(v, index){
        this.$http.get(API.serviceurl + 'service_categories/' + v + '/services').then((res) => {
          ...
          var articleRecomend = this.article.recommend[index]['tags3']
          var mcId = this.article.recommend[index]['name']
          var isTrue = articleRecomend[0].some((v) => {
            if (v.id == mcId) {
              return true
            } else {
              return false
          ...
          if (!isTrue) {
            this.article.recommend[index]['name'] = '';  // 清空select
          }
        }).catch()
      },
      // 获取所属分类
      getTags2(){
        if (this.article.category == 'policy_news') { //如果是政策快讯 就把下一个select清空
      ...
      onsubmit(formName){
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            this.$http.put(API.url + 'articles/' + this.$route.params.id, {
              article: {
