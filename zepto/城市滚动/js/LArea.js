window.LArea = (function() {
    ...
    MobileArea.prototype = {
        ...
        finish: function(e) {
            ...
            var v = provinceText.trim() + ((cityText)?(cityText.trim()):(''))+ ((countyText)?(countyText.trim()):(''));
            if(v.length > 14 && v.length < 18) {
            	v = v.replace(v.substr(6,3),"…");
            } else if(v.length > 18 ){
            	v = v.replace(v.substr(5,11),"…");
            }
            _self.trigger.value = v;
