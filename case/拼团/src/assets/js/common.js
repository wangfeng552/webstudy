...
import getServerDateService from '../../service/GetServerDateService';
import goShareService from '../../service/GoShareService';
...
export	var userUtil = {
	...
	checkLogin: function(callback, beforeLogin,status) {
		var that = this, url = '', isApp = !!navigator.userAgent.match(/jfmore/),loginUrl, source, source_val, _spm;;
...
//分享
export let  Share = {
	Weixin(callback){  //微信分享
			let h = location.href;
			let index = h.indexOf("#");
			if(index && index > 0) {
				h = h.substring(0,index);
			}
    goShareService.btnShare({
			authUrl:encodeURIComponent(h)
		})
				.then((res)=>{
					wx.config({
	...
	appShare(action,params,callback){
		if(browserUtil().android){
			this.Android(params,callback)
		}else{
			this.Ios(action,params,callback)
		}
	},
	...
	Android(params,callback) {  //安卓分享
    window.onShareSuccess = function(res) {
      callback(res);
    };
    jfNative.open('user', 'clickShare', params, 'onShareSuccess')
  }
}
...
//获取服务器当前时间
export var getServerDate = (callback) =>{
  ...
  getServerDateService.getServerTime()
		.then(res => {
			if(res.respCode == '0000'){
        that.__daTime = res.data.serverDate - (+new Date());
        that.__isAsync = true;
...
//app端分享图标初始化
export var shareInit = ()=>{
  var callback = null, $share;
  if (document.querySelector('body').getAttribute('data-needrefresh') == 'true') {
    browserUtil().checkRefresh(function() {
      location.reload();
    });
  }
  $share = document.querySelector('[data-sharemode]'); // 分享标识
  if (!browserUtil().jfmore || !$share)
    return;
  var rid, _run = function () {
    callback = 'call' + (Math.random() + '').slice(2);
    window[callback] = function(data) {
      delete window[callback];
    }
    if (browserUtil().ios) {
    	console.log('ios open')
      jfNative.bridge.check(function() {
        jfNative.bridge.open('hasShare', window._shareData, callback);
      })
    } else {
      jfNative.open('user', 'share', window._shareData, callback);
    }
  }
