    ...
    <div class="createnlayout_form">
      <el-form :model="enterprise" :rules="rules" ref="enterpriseForm" label-width="100px">
        ...
        <el-form-item v-show="toggleCity">
          <div class="wf_address">
            <v-distpicker :placeholders="placeholders_1" @province="selectProvince_1" @city="selectCity_1"
                          @area="selectArea_1"></v-distpicker>
          </div>
        </el-form-item>
        <el-form-item v-show="toggleCity">
        ...
        <el-form-item>
          ...
          <el-button type="primary" @click="onsubmit('enterpriseForm')" v-loading.fullscreen.lock="fullscreenLoading"
                     element-loading-text="注册中">确认
          </el-button>
        </el-form-item>
      </el-form>
    </div>
...
//        fd.append("user[invitor_id]", this.enterprise.invitor_id)
        ...
        if (this.toggleCity == true) {
          ...
          fd.append("corp[office_location]", this.enterprise.address5)
        } else {
          ...
          fd.append("corp[office_location]", this.enterprise.location)
        }
        /*图片*/
        ...
        this.$refs[formName].validate((valid) => {
          if (valid && !this.showAddressError) {
            ...
            this.$http.post(adminUrl.url + 'corps', fd).then(this._submitSuccess).catch(this._submitError)
          }
        });
      },
      // 上传图片
