<template>
  ...
  <div class="qy_register">
    <el-breadcrumb separator=">">
      <el-breadcrumb-item :to="{ name:'clientList' }">客户管理</el-breadcrumb-item>
      <el-breadcrumb-item>编辑企业</el-breadcrumb-item>
    </el-breadcrumb>
    <div class="createnlayout_form" v-if="corpsDtl">
      <el-form :model="corpsDtl" :rules="rules" ref="corpsDtl" label-width="100px">
        <el-form-item label="手机号码" prop="owner.phone_number">
          <el-input v-model="corpsDtl.owner.phone_number" placeholder="请输入11位手机号码"></el-input>
        </el-form-item>
        <el-form-item label="姓名" prop="owner.name">
          <el-input v-model="corpsDtl.owner.name" placeholder="请输入姓名"></el-input>
        </el-form-item>
        <el-form-item label="性别">
          <el-radio-group v-model="corpsDtl.owner.gender">
            <el-radio :label="true">男</el-radio>
            <el-radio :label="false">女</el-radio>
          </el-radio-group>
        </el-form-item>
        <el-form-item label="固定电话" prop="owner.fixed_phone">
          <el-input v-model="corpsDtl.owner.fixed_phone" placeholder="请输入固定电话 例:021-57431234"></el-input>
        </el-form-item>
        <el-form-item label="职位名称" prop="owner.position">
          <el-input v-model="corpsDtl.owner.position" placeholder="请输入职位名称"></el-input>
        </el-form-item>
        <!--<el-form-item label="邀请人编号">-->
        <!--<el-input v-model="enterprise.invitor_id" placeholder="请输入邀请人编号"></el-input>-->
        <!--</el-form-item>-->
        <el-form-item label="企业名称" prop="name">
          <el-input v-model="corpsDtl.name" placeholder="请填写与工商营业执照中相同的企业全称"></el-input>
        </el-form-item>
        <div class="el-form-item is-required">
          ...
          <div class="wf_address" :class="{wf_address_error:showAddressError}">
            <v-distpicker :placeholders="placeholders" @province="selectProvince" @city="selectCity"
                          @area="selectArea" :province="select.province" :city="select.city"
                          :area="select.area"></v-distpicker>
            <div class="el-form-item__error pf_shen">请选择企业地址</div>
          </div>
        </div>
        <el-form-item label="" prop="location">
          <el-input v-model="corpsDtl.location" placeholder="街道  门牌  建筑名  房间号"></el-input>
        </el-form-item>
        ...
        <el-form-item v-show="toggleCity">
          <div class="wf_address">
            <v-distpicker :placeholders="placeholders_1" @province="selectProvince_1" @city="selectCity_1"
                          @area="selectArea_1" :province="select.province_1" :city="select.city_1"
                          :area="select.area_1"></v-distpicker>
          </div>
        </el-form-item>
        <el-form-item v-show="toggleCity">
          <el-input v-model="corpsDtl.office_location" placeholder="街道  门牌  建筑名  房间号"></el-input>
        </el-form-item>
        ...
        <el-form-item>
          ...
          <el-button type="primary" @click="onsubmit('corpsDtl')" v-loading.fullscreen.lock="fullscreenLoading"
                     element-loading-text="上传中">确认
          </el-button>
        </el-form-item>
      </el-form>
    </div>
  ...
  export default {
    ...
    data(){
        ...
        if (!(/^1\d{10}$/.test(value))) {
          ...
          return false
            ...
            this.$http.get(adminUrl.url + 'users/valid_phone_number', {
              params: {
                phone_number: this.corpsDtl.owner.phone_number,
                type: 'Customer'
              }
            }).then((res) => {
      ...
      var checkCorpName = (rule, value, callback) => {
      	if(this.corpName == value ){
          callback();
        }else{
          this.$http.get(adminUrl.url + 'corps/valid_name', {
            params: {
              name: this.corpsDtl.name
            }
          }).then((res) => {
            var exist = res.body.exist
            if (exist) {
              callback(new Error('企业名称已被注册'))
            }
            else {
              callback()
            }
...
//              {required: true, message: '请输入手机号码', trigger: 'blur'},
       ...
       var item = [
          {
            ...
            url:this.imageUrl,
            name:this.imageUrlName,
            pdf:this.pfdFile
          },
          {
            category:"business",
            uploadType:this.uploadType_1,
            url:this.imageUrl_1,
            name:this.imageUrlName_1,
            pdf:this.pfdFile_1,

          },
          {
            category:"organization",
            uploadType:this.uploadType_2,
            url:this.imageUrl_2,
            name:this.imageUrlName_2,
            pdf:this.pfdFile_2,
          },
          {
            category:"tax",
            uploadType:this.uploadType_3,
            url:this.imageUrl_3,
            name:this.imageUrlName_3,
            pdf:this.pfdFile_3,
          }
        ]
    ...
    methods: {
      ...
      getCorp(){
        this.$http.get(adminUrl.url + 'customers/' + this.$route.params.id
        ).then((res) => {
          this.corpsDtl = res.body.customer
          this.phoneNum = this.corpsDtl.owner.phone_number
          this.corpName = this.corpsDtl.name

          this.select.province = this.corpsDtl.province
          this.select.city = this.corpsDtl.city
          this.select.area = this.corpsDtl.district
          this.select.province_1 = this.corpsDtl.office_province
          this.select.city_1 = this.corpsDtl.office_city
          this.select.area_1 = this.corpsDtl.office_district
          //获取营业执照
          const count = this.corpsDtl.certifications.length
          if(this.corpsDtl.certifications!= ''){
            if (count <= 1) {
            	const fileName = this.corpsDtl.certifications[0].name
            	if(this.hasPdf(fileName)){
                this.imageUrl= 'http://os8x5i7y1.bkt.clouddn.com/cover-pdf.png'
              }else{
                this.imageUrl = this.corpsDtl.certifications[0].url
              }
              this.toggleZhizao(0)
//              this.list_pic.push({key:this.corpsDtl.certifications[0].category,value:this.imageUrl});
            } else {
              this.corpsDtl.certifications.forEach((v,i) =>{
              	if(v.category == 'tax'){
              		const fileName_3 = v.name
                  if(this.hasPdf(fileName_3)){
                    this.imageUrl_3 = 'http://os8x5i7y1.bkt.clouddn.com/cover-pdf.png'
                  }else{
                    this.imageUrl_3 = v.url
                  }
                }else if(v.category == 'organization'){
                  const fileName_2 = v.name
                  if(this.hasPdf(fileName_2)){
                    this.imageUrl_2 = 'http://os8x5i7y1.bkt.clouddn.com/cover-pdf.png'
                  }else{
                    this.imageUrl_2 = v.url
                  }
                }else if(v.category == 'business'){
                  const fileName_1 = v.name
                  if(this.hasPdf(fileName_1)){
                    this.imageUrl_1 = 'http://os8x5i7y1.bkt.clouddn.com/cover-pdf.png'
                  }else{
                    this.imageUrl_1 = v.url
                  }
                }
              })
              ...
              console.log(this.corpsDtl);
               var that = this;
              this.corpsDtl.certifications.forEach(function (item,index) {
              	var obj ={};
                obj.category = item.category;
                if(0 == index){
                  obj.value = that.imageUrl_1;
                }
                else if(1 == index){
                  obj.value = that.imageUrl_2;
                }else if(2 == index){
                  obj.value = that.imageUrl_3;
                }
                console.log("=====");
//                that.list_pic.push(obj);
              });
          ...
          if (this.corpsDtl.address == this.corpsDtl.office_address) {
            this.enterprise.bgaddress = '与注册地址相同'
          } else {
            this.enterprise.bgaddress = '其他'
      ...
      onsubmit(formName) {
        ...
        /*用户*/
        fd.append("user[phone_number]", this.corpsDtl.owner.phone_number)
        fd.append("user[name]", this.corpsDtl.owner.name)
        fd.append("user[gender]", this.corpsDtl.owner.gender)
        fd.append("user[fixed_phone]", this.corpsDtl.owner.fixed_phone)
        fd.append("user[position]", this.corpsDtl.owner.position)
//        fd.append("user[invitor_id]", this.enterprise.invitor_id)
        /*企业*/
        fd.append("corp[name]", this.corpsDtl.name)
        fd.append("corp[province]", this.select.province)
        fd.append("corp[city]", this.select.city)
        fd.append("corp[district]", this.select.area)
        fd.append("corp[location]", this.corpsDtl.location)
        fd.append("corp[address_common]", this.toggleCity)
        fd.append("corp[cert_type]", this.yyzzlx)
        /*办公地址*/
        if (this.toggleCity == true) {
          fd.append("corp[office_province]", this.select.province_1)
          fd.append("corp[office_city]", this.select.city_1)
          fd.append("corp[office_district]", this.select.area_1)
          fd.append("corp[office_location]", this.corpsDtl.office_location)
        } else {
          fd.append("corp[office_province]", this.select.province)
          fd.append("corp[office_city]", this.select.city)
          fd.append("corp[office_district]", this.select.area)
          fd.append("corp[office_location]", this.corpsDtl.location)
        }
        /*图片*/
        ...
        var that = this;
        this.list_pic.forEach(function(item,index){

          if(item.uploadType == null || item.uploadType == undefined || item.uploadType.length<1){

          }else {
            if(item.uploadType == 'application/pdf'){
              fd.append("certifications[][name]", item.name)
              fd.append("certifications[][file]", item.pdf)

              if(item.category.length>1){
                fd.append("certifications[][category]", item.category)
              }


            }else if(item.uploadType == 'image/png' || item.uploadType == 'image/jpeg'){
              var blob = dataURItoBlob(item.url);
              fd.append("certifications[][name]", item.name)
...
//            if (!this.isContains(this.imageUrl, 'http')) {
...
//            }
//          } else {
//            if (!this.isContains(this.imageUrl_1, 'http') || !this.isContains(this.imageUrl_2, 'http') || !this.isContains(this.imageUrl_3, 'http')){
...
//        }
        ...
        this.$refs[formName].validate((valid) => {
          if (valid && !this.showAddressError) {
            ...
            this.$http.put(adminUrl.url + 'customers/' + this.$route.params.id, fd).then(this._submitSuccess).catch(this._submitError)
          }
        });
      },
      // 上传图片
