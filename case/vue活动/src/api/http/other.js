...
instance.interceptors.response.use(({ data: { code, data }}) =>{
  if (code != 200){
...
export async function getCaptcha(){
  return instance2('/exchange/codes/captcha', { responseType: 'arraybuffer' })
    .then((response) => {
      const image = btoa(
      ...
      return {
        ...
        data: `data:${response.headers['content-type'].toLowerCase()};base64,${image}`
      }
    })
}
