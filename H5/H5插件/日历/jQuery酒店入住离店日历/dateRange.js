...
pickerDateRange.prototype.close = function(btnSubmit) {
	...
	if(btnSubmit){
		...
		//$("#xgtianshu").text('共'+(-step || 0)+'晚');
		$(".count_date").text((-step || 0));
		// 更改目标元素值
	...
	$(".gravity_data_popup").css('display', 'none');
	if(_val !== "" && _val != undefined){
		var dateArr = _val.split(",");
		$(".m_rz").html(getMd(dateArr[0]));
		$(".m_ld").html(getMd(dateArr[1]));
		$("#ruzhuTime").val(function(){
			return getMd(dateArr[0],true)
		});
		$("#lidianTime").val(function(){
			return getMd(dateArr[1],true)
		});
	}
...
pickerDateRange.prototype.fillDate = function(year, month, index) {
    ...
    for(var d = dateBegin; d.getTime() <= dateEnd.getTime(); d.setDate(d.getDate() + 1)) {
        if(d.getTime() < firstDayOfMonth.getTime()) { // 当前月之前的日期
            ...
            tdClass = this.mOpts.theme + '_' + this.mOpts.disableGray;
            deviation = '1';
        } else if((this.mOpts.stopToday == true && d.getTime()+(1000*60*60*24) < today.getTime()) || d.getTime() < __method.mOpts.minValidDate * 1000 || ('' !== __method.mOpts.maxValidDate && d.getTime() > __method.mOpts.maxValidDate * 1000)) { // 当前时间之后的日期，或者开启统计之前的日期
            tdClass = this.mOpts.theme + '_' + this.mOpts.disableGray;
            deviation = '2';
        } else { // 当前月日期
            deviation = '0';
