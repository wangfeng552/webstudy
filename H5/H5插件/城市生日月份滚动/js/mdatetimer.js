(function($) {
	...
	(function(g, n, f) {
		...
		p.prototype = {
			...
			_transitionTime: function(a) {
				a = a || 0;
				this.scrollerStyle[d.style.transitionDuration] = 500 + "ms";
	...
	$.fn.mdatetimer = function(J) {
		...
		var F = {
			...
			getLastDayInMonth: function(a, b) {
				return new Date(a, b, this.getDaysInMonth(a, b))
			},
			preZero: function(a) {
				a = parseInt(a);
				if (a < 10) {
			...
			formatDate: function(a, b, c, d, e, f) {
				b = F.preZero(b + 1);
				c = F.preZero(c);
				d = F.preZero(d);
				e = F.preZero(e);
			...
			getDateValue: function(a) {
				if (L.format == 2) {
					...
					var b = a.replace(/\D ?/g, ',').slice(0, -1).split(',');
					if (b.length == 7) {
						if (a.indexOf('上午') >= 0) {
							return new Date(b[0], b[1] - 1, b[2], b[3], b[4])
						} else {
							return new Date(b[0], b[1] - 1, b[2], parseInt(b[3]) + 12, b[4])
						}
					} else {
		...
		var O = {
			init: function() {
				...
				var h = $('.mt_poppanel'),
					...
					mpDay = $('.mt_day', h),
					mpHour = $('.mt_hour', h),
					mpMinute = $('.mt_minute', h),
					mpSecond = $('.mt_second', h);
				var o = N.val() != '' ? F.getDateValue(N.val()) : new Date(),
					dYear = o.getFullYear(),
					dMonth = o.getMonth()+1,
					dDate = o.getDate(),
					dHour = o.getHours(),
					dMinute = o.getMinutes(),
					dSecond = o.getSeconds();
				var p = '';
				for (var i = 0; i < L.years.length; i++) {
					var y = L.years[i];
					var q = y == dYear ? 'selected' : '';
					p += '<li class="' + q + '" data-year="' + y + '">' + y + '年</li>'
				}
				p += '<li></li><li></li>';
				mpYear.find('ul').append(p);
				var r = '';
				for (var j = 1; j <= 12; j++) {
					var q = j == dMonth ? 'selected' : '';
					r += '<li class="' + q + '" data-month="' + (j - 1) + '">' + F.preZero(j) + '月</li>'
				}
				r += '<li></li><li></li>';
				mpMonth.find('ul').append(r);
				var s = '';
				var t = F.getDaysInMonth(dYear, dMonth);
				for (var k = 1; k <= t; k++) {
					var q = k == dDate ? 'selected' : '';
					s += '<li class="' + q + '" data-day="' + k + '">' + F.preZero(k) + '日</li>'
				}
				s += '<li></li><li></li>';
				mpDay.find('ul').append(s);
				if (L.mode == 1) {
					$('.mt_body', h).css('height', 200);
					$('.mt_year, .mt_month, .mt_day', h).css('height', '100%');
					$('.mt_sepline, .mt_hour, .mt_minute, .mt_second, .mt_indicate.cate2', h).remove()
				}
				if (!L.nowbtn) {
					$('.mt_setnow', h).remove();
					$('.mt_cancel, .mt_ok', h).css('float', 'none');
					$('.mt_cancel', h).css('border-right', '1px solid #4eccc4')
				}
				document.addEventListener('touchmove', function(e) {
					e.preventDefault()
				}, false);
				var u = itemHeight;
				var v = new IScroll('.mt_year', M);
				v.on('scroll', function() {
					g.updateSelected(mpYear, this)
				});
				v.on('scrollEnd', function() {
					g.updateSelected(mpYear, this);
					g.updateDay(mpYear, mpMonth, mpDay)
				});
				var w = new IScroll('.mt_month', M);
				w.on('scroll', function() {
					g.updateSelected(mpMonth, this)
				});
				w.on('scrollEnd', function() {
					g.updateSelected(mpMonth, this);
					g.updateDay(mpYear, mpMonth, mpDay)
				});
				var x = new IScroll('.mt_day', M);
				x.on('scroll', function() {
					g.updateSelected(mpDay, this)
				});
				x.on('scrollEnd', function() {
					g.updateSelected(mpDay, this)
				});
				this.yearScroll = v;
				this.monthScroll = w;
				...
				if (L.mode != 1) {
					var z = '';
					var A = L.mode == 3 ? 12 : 24;
					for (var l = 1; l <= A; l++) {
						var q = l == dHour ? 'selected' : '';
						z += '<li class="' + q + '" data-hour="' + l + '">' + F.preZero(l) + '时</li>'
					}
					z += '<li></li><li></li>';
					mpHour.find('ul').append(z);
					var B = new IScroll('.mt_hour', M);
					B.on('scroll', function() {
						g.updateSelected(mpHour, this)
					});
					...
					var C = '';
					for (var m = 0; m <= 60; m++) {
						var q = m == dMinute ? 'selected' : '';
						C += '<li class="' + q + '" data-minute="' + m + '">' + F.preZero(m) + '分</li>'
					}
					C += '<li></li><li></li>';
					mpMinute.find('ul').append(C);
					var D = new IScroll('.mt_minute', M);
					D.on('scroll', function() {
						g.updateSelected(mpMinute, this)
					});
					...
					if (L.mode == 4) {
						for (var n = 0; n <= 60; n++) {
							var q = n == dSecond ? 'selected' : '';
							E += '<li class="' + q + '" data-second="' + n + '">' + F.preZero(n) + '秒</li>'
						}
					} else if (L.mode == 3) {
						var G = dHour <= 12 ? 'selected' : '';
						var H = dHour >= 12 ? 'selected' : '';
						E += '<li class="' + G + '" data-second="am">上午</li><li class="' + H + '" data-second="pm">下午</li>'
					}
					E += '<li></li><li></li>';
					mpSecond.find('ul').append(E);
					var I = new IScroll('.mt_second', M);
					I.on('scroll', function() {
						g.updateSelected(mpSecond, this)
					});
				...
				N.on('click', function() {
					if (h.hasClass('show')) {
						...
						g.showPanel();
						var a = mpYear.find('.selected').data('year');
						var b = mpMonth.find('.selected').data('month');
						var c = mpDay.find('.selected').data('day');
						var d = mpHour.find('.selected').data('hour');
						var e = mpMinute.find('.selected').data('minute');
						var f = mpSecond.find('.selected').data('second');
						g.setValue(a, b, c, d, e, f)
					}
				});
				...
				$('.mt_ok', h).on('tap', function() {
					...
					var e = mpMinute.find('.selected').data('minute');
					var f = mpSecond.find('.selected').data('second');
					N.val(F.formatDate(a, b, c, d, e, f));
					g.hidePanel();
					L.onOk && typeof L.onOk == 'function' && L.onOk(h)
				});
				$('.mt_cancel', h).on('tap', function() {
					g.hidePanel();
					L.onCancel && typeof L.onCancel == 'function' && L.onCancel(h)
				});
				$('.mt_setnow', h).on('tap', function() {
					var n = new Date();
					N.val(F.formatDate(n.getFullYear(), n.getMonth(), n.getDate(), n.getHours(), n.getMinutes(), n.getSeconds()));
					g.hidePanel()
				});
			...
			setValue: function(y, m, d, h, a, s) {
				var b = $('.mt_year li[data-year="' + y + '"]'),
					monthItem = $('.mt_month li[data-month="' + m + '"]'),
					dayItem = $('.mt_day li[data-day="' + d + '"]'),
					hourItem = $('.mt_hour li[data-hour="' + h + '"]'),
					minuteItem = $('.mt_minute li[data-minute="' + a + '"]'),
			...
			updateDay: function(b, c, d) {
				var e = this;
				var f = b.find('li.selected').data('year');
				var g = c.find('li.selected').data('month');
				var h = d.find('li.selected').data('day');
				var i = F.getDaysInMonth(f, g);
				var j = '<li class="mt_note">选择日期</li><li></li>';
				for (var k = 1; k <= i; k++) {
					var l = k == h ? 'selected' : '';
					j += '<li class="' + l + '" data-day="' + k + '">' + F.preZero(k) + '日</li>'
				}
				j += '<li></li><li></li>';
				d.find('ul').html(j);
				this.dayScroll.destroy();
				this.dayScroll = new IScroll('.mt_day', M);
				this.dayScroll.on('scroll', function() {
					e.updateSelected(d, this)
				});
				this.dayScroll.on('scrollEnd', function() {
					e.updateSelected(d, this)
				});
				setTimeout(function() {
					var a = d.find('li[data-day="' + h + '"]');
					if (a.length > 0) {
						e.checkDay(a)
					}
				}, 10)
			...
			checkYear: function(a) {
				if (a.text() == '') return;
				var b = a.prev('li').prev('li');
				this.yearScroll.scrollToElement(b[0])
			},
			checkMonth: function(a) {
				if (a.text() == '') return;
				var b = a.prev('li').prev('li');
				this.monthScroll.scrollToElement(b[0])
			},
			checkDay: function(a) {
				if (a.text() == '') return;
				var b = a.prev('li').prev('li');
				this.dayScroll.scrollToElement(b[0])
			},
			checkHour: function(a) {
				if (a.text() == '') return;
				var b = a.prev('li').prev('li');
				this.hourScroll.scrollToElement(b[0])
			},
			checkMinute: function(a) {
				if (a.text() == '') return;
				var b = a.prev('li').prev('li');
				this.minuteScroll.scrollToElement(b[0])
			},
			checkSecond: function(a) {
				if (a.text() == '') return;
				if (L.mode < 3) return;
				var b = a.prev('li').prev('li');
				this.secondScroll.scrollToElement(b[0])
			}
		}
		O.init();
		return O
