...
(function($) {
    ...
    var ajaxUtil = {
        loadData: function(options) {
			...
			if (options.standard) {
				__postData = {};
				__postData.reqTime = postData.reqTime;
				delete postData.reqTime;
				__postData.data = postData;
				for (var item in options.params) {
					__postData.data[item] = options.params[item];
				}
				postData = __postData;
			} else {
				for (var item in options.params) {
			...
			postData.sign = stringUtil.createSign(options.standard ? postData.data : postData, {signKey:options.type=='payload'?'123456':''});
            //
            if ($) {
                var ajaxSetting = {
    ...
    //
    window.loadAjaxData = function(url, params, onSuccess, onError, mockCount, standard) {
    ...
    var page = {
        ...
        initHead: function() {
            ...
            if (browserUtil.jfmore) {
                ...
                //
                if (sharemode!='' && browserUtil.jfmore != '1.0.0') {
